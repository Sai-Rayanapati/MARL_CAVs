...
class MultiMergeEnv(AbstractEnv):
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
       ...
       """
        ...
        if vehicle.lane_index == ("b", "c", 1) or vehicle.lane_index == ("b", "c", 2):
    ...
    def _regional_reward(self):
        for vehicle in self.controlled_vehicles:
            ...
            if vehicle.lane_index == ("a", "b", 0) or vehicle.lane_index == ("b", "c", 0) or vehicle.lane_index == (
                ...
                #Second merge lane
                elif vehicle.lane_index == ("n", "b", 0):
                    v_fl, v_rl = self.road.surrounding_vehicles(vehicle, ("k", "b", 0))
    ...
    def _make_road(self, ) -> None:
        ...
        # First Merging lane
        amplitude = 3.25
        ljk = StraightLane([0, 6.5 + 4], [self.ends[0], 6.5 + 4], line_types=[c, s], forbidden=False)
        
        lkb_start = ljk.position(self.ends[0], -amplitude)
        lkb_end = ljk.position(sum(self.ends[:2]), -amplitude)
        lkb = SineLane(lkb_start, lkb_end,
                       amplitude, 2 * np.pi / (2 * self.ends[1]), np.pi / 2, line_types=[c, s], forbidden=False)
        
        lbc_start = lkb.position(self.ends[1], 0)
        lbc_end = lkb.position(self.ends[1], 0) + [self.ends[2], 0]
        lbc = StraightLane(lbc_start, lbc_end, line_types=[n, s], forbidden=False)
        ...
        # Second Merging lane
        amplitude = 3.25
        lmn = StraightLane([0, 14.5], [self.ends[0], 14.5], line_types=[n, c], forbidden=True)
        
        lnb_start = lmn.position(self.ends[0], -amplitude)
        lnb_end = lmn.position(sum(self.ends[:2]), -amplitude)
        lnb = SineLane(lnb_start, lnb_end,
                        amplitude, 2 * np.pi / (2 * self.ends[1]), np.pi / 2, line_types=[n, c], forbidden=True)
        
        lbc2_start = lnb.position(self.ends[1], 0)
        lbc2_end = lnb.position(self.ends[1], 0) + [self.ends[2], 0]
        lbc2 = StraightLane(lbc2_start, lbc2_end,
                            line_types=[n, c], forbidden=True)
        
        net.add_lane("m", "n", lmn)
        net.add_lane("n", "b", lnb)
        net.add_lane("b", "c", lbc2)       

        road = Road(network=net, np_random=self.np_random, record_history=self.config["show_trajectories"])
        
        #Add obstacles for merging lane end
        road.objects.append(Obstacle(road, lbc.position(self.ends[2], 0))) #Obstacle for First Merge lane
        road.objects.append(Obstacle(road, lbc2.position(self.ends[2], 0))) #Obstacle for Second Merge lane        
        self.road = road
    ...
    def _make_vehicles(self, num_CAV=4, num_HDV=3) -> None:
        ...
        for _ in range(num_CAV // 2):
            ego_vehicle = self.action_type.vehicle_class(road, road.network.get_lane(("a", "b", 0)).position(
                spawn_point_s_c.pop(0) + loc_noise.pop(0), 0), speed=initial_speed.pop(0), enable_lane_change=False)
        ...
        for _ in range(num_CAV - num_CAV // 2):
            if random.random() < 0.5:
                ...
                ego_vehicle = self.action_type.vehicle_class(road, road.network.get_lane(("m", "n", 0)).position(
                    spawn_point_m_c.pop(0) + loc_noise.pop(0), 0), speed=initial_speed.pop(0))
        ...
        for _ in range(num_HDV // 2):
            veh = other_vehicles_type(road, road.network.get_lane(("a", "b", 0)).position(
                    spawn_point_s_h.pop(0) + loc_noise.pop(0), 0),
                                    speed=initial_speed.pop(0), enable_lane_change=False)
            #print(dir(veh))
            ...
            if random.random() < 0.5:
                veh = other_vehicles_type(road, road.network.get_lane(("j", "k", 0)).position(
                        spawn_point_m_h.pop(0) + loc_noise.pop(0), 0),
                                        speed=initial_speed.pop(0), enable_lane_change=True)
            else:
                veh = other_vehicles_type(road, road.network.get_lane(("m", "n", 0)).position(
                        spawn_point_m_h.pop(0) + loc_noise.pop(0), 0),
                                        speed=initial_speed.pop(0), enable_lane_change=True)
            road.vehicles.append(veh)                
...
class MultiMergeEnvMARL(MultiMergeEnv):
    @classmethod
    def default_config(cls) -> dict:
        config = super().default_config()
        config.update({
...
register(
    id='multi-merge-v0',
    entry_point='highway_env.envs:MultiMergeEnv',
)

register(
    id='multi-merge-multi-agent-v0',
    entry_point='highway_env.envs:MultiMergeEnvMARL',
)